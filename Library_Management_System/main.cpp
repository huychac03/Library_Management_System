...
int main()
{
	...
	Menu* memReadMenu = new Menu("Member read menu", "You just read a book in the library", MEM_READ);
	...
	Menu* subCollectionMenu = new Menu("Sub Collection Menu", "Now, that collection is in ur Collection list", MEM_SUB_COLLECTION);
	memberMenu->addSubMenu(subCollectionMenu);

	Menu* unSubCollectionMenu = new Menu("Un Sub Collection Menu", "Now, that collection is deleted from ur Collection list", MEM_UNSUB_COLLECTION);
	memberMenu->addSubMenu(unSubCollectionMenu);

	Menu* borrowMenu = new Menu("Borrow Menu", "You just borrow a book", MEM_BORROW);
	memberMenu->addSubMenu(borrowMenu);

	Menu* returnMenu = new Menu("Return Menu", "You just return a book", MEM_RETURN);
